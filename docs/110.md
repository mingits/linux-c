...
1. 用 `gdb` 一步一步跟踪[第 5 章第 3 节「递归」](105#_3-递归)讲的 `factorial` 函数，对照着[第 5 章第 3 节图 5.2 「factorial (3) 的调用过程」](105#c5-2)查看各层栈帧的变化情况，练习本节所学的各种 `gdb` 命令。
...
> <sup>[22]</sup> 打印结果最左边的一长串数字是内存地址，在[第 17 章第 1 节「内存与地址」](217#_1-内存与地址)详细解释，目前可以无视。
